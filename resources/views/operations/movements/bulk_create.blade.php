...
@section('title', 'Record Sample Movement - TradeSyncERP')
...
<div class="nxl-content">
    <div class="page-header">
        <div class="page-header-left d-flex align-items-center">
            <div class="page-header-title"><h5 class="m-b-10">Record Sample Movement</h5></div>
            <ul class="breadcrumb">
        ...
        <div class="page-header-right ms-auto">
            <div class="page-header-right-items">
                <div class="d-flex align-items-center gap-2 page-header-right-items-wrapper">
                    ...
                    <button type="submit" form="bulkMovementForm" class="btn btn-primary">
                        <i class="feather-save me-2"></i>Save Movement
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="main-content">
        ...
        @if($preselectedRun)
        <div class="alert alert-info border-0 mb-4 d-flex align-items-center gap-3">
            <i class="feather-info fs-5"></i>
            <div>
                Linked to <strong>Inspection Run</strong> from
                <strong>{{ $preselectedRun->inspection->report_number }}</strong>.
                Samples and employees have been pre-populated below.
            </div>
        </div>
        @endif

        <form id="bulkMovementForm" action="{{ route('movements.store') }}" method="POST">
            @csrf

            @if($preselectedRun)
            <input type="hidden" name="inspection_run_id" value="{{ $preselectedRun->id }}">
            @endif
                    ...
                    <div class="card mb-4">
                        <div class="card-header d-flex align-items-center justify-content-between">
                            <h5 class="card-title mb-0">
                                Samples <span class="text-danger">*</span>
                            </h5>
                        ...
                        <div class="card-body">
                                ...
                                <table class="table table-sm table-hover mb-0">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th class="text-end" style="width:82px">Available</th>
                            ...
                            <div id="noSamplesMsg" class="text-muted fs-12 mt-1">No samples added yet.</div>
                        </div>
                    </div>
                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <select name="inspection_run_id" id="inspectionRunSelect"
                                ...
                                <option value="{{ $run->id }}" @selected(old('inspection_run_id') == $run->id)>
                                    {{ $run->inspection->report_number ?? 'Inspection #'.$run->inspection_id }}
                                    — Run {{ $run->run_number ?? $run->id }}
                                </option>
                                @endforeach
                            </select>
                            <div class="form-text">Selecting a run auto-fills assigned employees.</div>
...
const PRESELECTED_SAMPLE_IDS   = @json($preselectedRun ? $preselectedRun->inspection->samples->pluck('id') : []);
const PRESELECTED_EMPLOYEE_IDS = @json($preselectedEmployeeIds);

// ── State ─────────────────────────────────────────────────────────────────────
let addedSamples   = new Set();
let addedEmployees = new Set();
let itemCounter    = 0;   // increments per variation row (never resets — gaps are fine for PHP)
let sampleCount    = 0;   // visible sample group counter, renumbered on remove
let employeeCount  = 0;   // visible employee row counter, renumbered on remove
...
function addSample(sampleId) {
    ...
    if (s.variations && s.variations.length > 0) {
        ...
        singleRow.innerHTML = `
            ...
            <td></td>
            <td colspan="3" class="ps-3 text-muted fs-12 fst-italic">No variations defined</td>
            <td class="text-center">
                <input type="number" name="items[${idx}][quantity]"
                       class="form-control form-control-sm variation-qty mx-auto"
                       min="0" value="1">
...
function renumberSampleGroups() {
    ...
    samplesTableBody.querySelectorAll('tr.sample-group-header').forEach(tr => {
        tr.querySelector('.sample-num').textContent = ++n;
    });
    sampleCount = n;
}
